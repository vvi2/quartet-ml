...
from quartet_ml.baselines import iq_tree
...
def evaluate_iqtree():
  ...
  for i, (_, row) in enumerate(test_df.iterrows()):
    ...
    output_prefix = Path(row["filepath"]).stem
    pred_label = iq_tree(row["filepath"], output_prefix)
    regime = row["regime"]
  ...
  pd.DataFrame({"method": "IQ-TREE", "regime": regimes, "internal_branch_length": internal_branch_lengths, "correct": correct}).to_csv(headline_file_path / "iqtree.csv", index=False)
  gtr_y_true = []
  gtr_y_pred = []
  ...
  for gtr_i, (_, gtr_row) in enumerate(df_gtr.iterrows()):
    ...
    gtr_output_prefix = Path(gtr_row["filepath"]).stem
    gtr_pred_label = iq_tree(gtr_row["filepath"], gtr_output_prefix)
