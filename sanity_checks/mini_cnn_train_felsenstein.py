...
from quartet_ml.train import CNN, get_cnn_split_data, prepare_cnn_dataloader, train_cnn_with_early_stopping

def mini_cnn_train_felsenstein():
  train_x, train_y = get_cnn_split_data("train", regime_type="felsenstein")
  val_x, val_y = get_cnn_split_data("val", regime_type="felsenstein")
  ...
  _, model_history, metrics = train_cnn_with_early_stopping(cnn_model, train_loader, val_loader)
  ...
  fig.tight_layout()
  fig.savefig(Path(__file__).resolve().parent / "mini_cnn_train_felsenstein.png")
  plt.show()
  return metrics

if __name__ == "__main__":
  metrics = mini_cnn_train_felsenstein()
  print(metrics)
